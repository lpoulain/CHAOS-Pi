...
//    unsigned char *ptr_to_ptr;
    ...
    elf->relocation_offset = (int64)elf->section[ELF_SECTION_TEXT].start - elf->initial_code_address;
...
Elf *elf_load(const char *filename, uint dir_cluster) {
    ...
    if (strncmp((char*)(f->body + 1), "ELF", 3)) {
    ...
    ElfSectionHeader *sections = (ElfSectionHeader*)((uint64)elf->header + elf->header->e_shoff);
    ...
    for (int idx = 0; idx < elf->header->e_shnum; idx++) {
        ...
        if (section->sh_name) {
            section_number = -1;
            section_name = (char*)(elf->section[ELF_SECTION_STRING].start + section->sh_name);
