...
//					dwarf_end_sequence = 1;
				...
				// DW_LNE_set_address
				case 2:
					dwarf_address = *((unsigned char **)(opcode + 3));
					dwarf_line = 1;
					break;
				// DW_LNE_define_file
				case 3:
...
//					dwarf_isa = decodeULEB128(opcode + 1, &nb_bytes);
	...
	char *filename = (char*)files;
	char *path = (char*)dirs;
	for (int i=0; i<0; i++) {
		files += strlen((char*)files) + 2;
	}
//	debug(filename);
	filename = (char*)(files + strlen((char*)files) + 1);
...
//			printf("Address: %x\n", *address);
			if (*address > (uint64)ptr) return old_block;
		}
