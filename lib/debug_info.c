			...
			while ((*schema != 0 || *(schema-1) != 0)) {
				...
				if (is_function_type) {
					if (*(schema-1) == DW_AT_name || *(schema-1) == DW_AT_linkage_name) {
						if (*schema == DW_FORM_string) functionRanges[nb_function_ranges].name = (char *)die;
						else functionRanges[nb_function_ranges].name = (char*) (elf->section[ELF_SECTION_DEBUG_STR].start + *(uint*)die);
					}
...
//								printf("[%s : %x->%x]\n", functionRanges[i].name, functionRanges[i].low_pc, functionRanges[i].low_pc+functionRanges[i].high_pc);
				...
				// It has a name. Add an entry and unmangle the name if it is a C++ name
					...
					functionRanges[nb_function_ranges].address = (uint64)(die_start - compilation_unit_start);
...
//	return;
//for (;;);
	kheap_check_for_corruption("Problem");
